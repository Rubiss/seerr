...
export class AddForeignKeyIndexes1771259406751 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const hasBlocklistTmdbId =
      hasBlocklist && (await queryRunner.hasColumn('blocklist', 'tmdbId'));
    const hasBlocklistExternalId =
      hasBlocklist && (await queryRunner.hasColumn('blocklist', 'externalId'));

    if (hasBlocklist) {
      await queryRunner.query(
        `ALTER TABLE "blocklist" DROP CONSTRAINT IF EXISTS "FK_53c1ab62c3e5875bc3ac474823e"`
      );
      await queryRunner.query(
        `ALTER TABLE "blocklist" DROP CONSTRAINT IF EXISTS "FK_62b7ade94540f9f8d8bede54b99"`
      );
      await queryRunner.query(
        `DROP INDEX IF EXISTS "public"."IDX_6bbafa28411e6046421991ea21"`
      );
      await queryRunner.query(
    ...
    if (hasBlocklistTmdbId) {
      await queryRunner.query(
        `CREATE INDEX IF NOT EXISTS "IDX_09b94c932e84635c5461f3c0a9" ON "blocklist" ("tmdbId") `
      );
    } else if (hasBlocklistExternalId) {
      await queryRunner.query(
    ...
    if (
      ...
      !(await this.constraintExists(
        queryRunner,
        'blocklist',
        'FK_356721a49f145aa439c16e6b999'
      ))
    ) {
      await queryRunner.query(
    ...
    if (
      ...
      !(await this.constraintExists(
        queryRunner,
        'blocklist',
        'FK_5c8af2d0e83b3be6d250eccc19d'
      ))
    ) {
      await queryRunner.query(

    ...
    try {
      ...
      if (tvShow) {
        await this.asyncLock.dispatch(tvShow.id, async () => {
          ...
          const seasonHasEpisodes = (seasonNumber: number): boolean =>
            (tvShow?.seasons.find((s) => s.season_number === seasonNumber)
              ?.episode_count ?? 0) > 0;
          ...
          if (media) {
            ...
            const countsTowardsRollup = (
              ...
              const scannedSeason = tvShow?.seasons.find(
                (s) => s.season_number === season.seasonNumber
              );

              if (scannedSeason) {
                return scannedSeason.episode_count > 0;
              }

              return season[statusKey] !== MediaStatus.UNKNOWN;
            };

            const standardSeasonsForRollup = nonSpecialSeasons.filter(
              (season) => countsTowardsRollup(season, 'status')
            );
            ...
            const seasons4kForRollup = nonSpecialSeasons.filter((season) =>
              countsTowardsRollup(season, 'status4k')
            );
            ...
            const newSeasonsForRollup = newSeasons.filter(
              (season) =>
                season.seasonNumber !== 0 &&
                seasonHasEpisodes(season.seasonNumber)
            );
            const isAllStandardSeasonsAvailable =
            ...
            const newMedia = new Media({
              ...
              jellyfinMediaId: isAllStandardSeasonsAvailable ? Id : null,
              jellyfinMediaIdAlt:
                isAll4kSeasonsAvailable && this.enable4kShow ? Id : null,
